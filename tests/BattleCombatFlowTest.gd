...
func _ready() -> void:
	...
	var point_blank_spacing := player1.pushbox_half_width() + player2.pushbox_half_width()
	await _run_move_test("high", false, point_blank_spacing, "head", "point-blank normal high lands")
	await _run_move_test("mid", false, point_blank_spacing, "arm_r", "point-blank normal mid lands")
	await _run_move_test("low", false, point_blank_spacing, "leg_r", "point-blank normal low lands")
	await _run_move_test("high", true, point_blank_spacing, "head", "point-blank heavy high lands")
	await _run_move_test("mid", true, point_blank_spacing, "arm_r", "point-blank heavy mid lands")
	await _run_move_test("low", true, point_blank_spacing, "leg_r", "point-blank heavy low lands")
...
func _run_move_test(band: String, heavy: bool, spacing: float, target: String, label: String) -> void:
	...
	var move: FighterMoveData = player1._attack_data(band, heavy).move_resource
	Input.action_press("p1_" + band)
	await get_tree().physics_frame
	Input.action_release("p1_" + band)
	Input.action_release("p1_heavy")
	await _wait_physics_frames(move.startup_frames + move.active_frames + 5)
...
func _test_real_pushboxes() -> void:
	...
	var required_spacing := player1.pushbox_half_width() + player2.pushbox_half_width()
	_expect_true(final_spacing >= required_spacing - 0.1, "grounded pushboxes never overlap (spacing %.1f / required %.1f)" % [final_spacing, required_spacing])
...
func _run_legless_target_test(band: String, spacing: float, target: String, label: String) -> void:
	...
	await _wait_physics_frames(2)
	var hp_before := float(player2.limb_hp[target].hp)
	var move: FighterMoveData = player1._attack_data(band, false).move_resource
	Input.action_press("p1_" + band)
	await get_tree().physics_frame
	Input.action_release("p1_" + band)
	await _wait_physics_frames(move.startup_frames + move.active_frames + 5)
