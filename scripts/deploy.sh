...
echo "==> [3/4] Validating project loads (headless) on remote..."
ssh "${REMOTE_HOST}" "cd ${REMOTE_DIR} && ${GODOT} --headless --path . --quit"

echo "==> [4/4] Launching the game on remote for a test..."
ssh "${REMOTE_HOST}" "cd ${REMOTE_DIR} && nohup ${GODOT} --path . > /tmp/fighter_run.log 2>&1 & echo \$! > /tmp/fighter.pid; sleep 4; if kill -0 \$(cat /tmp/fighter.pid) 2>/dev/null; then echo 'GAME LAUNCHED (pid '\$(cat /tmp/fighter.pid)')'; else echo 'game exited early, log:'; cat /tmp/fighter_run.log; fi"
